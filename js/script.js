...
this.addEventListener('load', () => {
  ...
  const arrRusKeys = document.querySelectorAll('.rus');
  const arrEngKeys = document.querySelectorAll('.eng');
  const arrKeyboardKey = document.querySelectorAll('.keyboard-key');
  const arrKeyboardKeyGreen = document.querySelectorAll('.green');
  const textaria = document.querySelector('.textarea');
  //   функция для capslock
  function capsLock() {
    for (let i = 0; i < arrEngKeys.length; i += 1) {
    ...
    for (let i = 0; i < arrRusKeys.length; i += 1) {
  ...
  document.querySelector('#Backspace').addEventListener('click', () => {
    if (textaria.selectionStart === 0) {
      ...
      textaria.innerHTML = textaria.value.slice(0, textaria.selectionStart - 1)
      + textaria.value.slice(textaria.selectionStart, textaria.value.length);
    }
  });
  //   функция для delete
  document.querySelector('#Delete').addEventListener('click', () => {
    textaria.innerHTML = textaria.value.slice(0, textaria.selectionStart)
    + textaria.value.slice(textaria.selectionStart + 1, textaria.value.length);
  });
  //   функция для Enter
  const lineBreak = '\n';
  document.querySelector('#Enter').addEventListener('click', () => {
    textaria.innerHTML = textaria.value.substring(0, textaria.selectionStart)
    + lineBreak
  ...
  //   клик на все кнопки
  for (let i = 0; i < arrKeyboardKey.length; i += 1) {
        ...
        if (arrKeyboardKey[i].className === 'keyboard-key') {
          if (arrKeyboardKey[i].children[0].className === 'rus') {
            if (
              ...
              const newSymbol = arrKeyboardKey[i].children[0].children[0].innerHTML;
              textaria.innerHTML += newSymbol;
            }
            if (arrKeyboardKey[i].children[0].children[2].className === 'caps') {
              const newSymbol = arrKeyboardKey[i].children[0].children[2].innerHTML;
              textaria.innerHTML += newSymbol;
            }
          }
          if (arrKeyboardKey[i].children[1].className === 'eng') {
            if (
              ...
              const newSymbol = arrKeyboardKey[i].children[1].children[0].innerHTML;
              textaria.innerHTML += newSymbol;
            }
            if (arrKeyboardKey[i].children[1].children[2].className === 'caps') {
              const newSymbol = arrKeyboardKey[i].children[1].children[2].innerHTML;
              textaria.innerHTML += newSymbol;
            }
          }
        }
  ...
  document.addEventListener('keyup', (e) => {
    ...
    for (let i = 0; i < arrKeyboardKeyGreen.length; i += 1) {
